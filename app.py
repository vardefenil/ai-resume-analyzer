...
def create_demo_user():
    db = SessionLocal()
    try:
        existing = db.query(models.User).filter(models.User.email == "demo@resumeanalytics.com").first()
        if not existing:
            demo = models.User(
                full_name="Demo User",
...
def signup(data: UserSignup, db: Session = Depends(get_db)):
    ...
    return {"status": "success", "user": {"id": new_user.id, "email": new_user.email, "full_name": new_user.full_name}}
...
def get_stats(db: Session = Depends(get_db)):
    ...
    total_logins = db.query(func.count(models.User.id)).scalar() or 0
